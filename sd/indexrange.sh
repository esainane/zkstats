...
main() {
    local start="$1"
    local end="$2"
    seq "$start" 40 "$end" | xargs -n1 -d \\n -I {} curl -s "https://zero-k.info/Battles?Title=MM+&Map=&PlayersFrom=2&PlayersTo=2&Age=0&Mission=2&Bots=2&Rank=8&Victory=0&Offset={}" | sed -n "s_^.*<a href='/Battles/Detail/\([0-9]\+\)'.*\$_\1_p" | grep -vxFf demos/exclude.txt | paste -d\  -s - | sed 's/^/BATTLEIDS:=/' > demos/index.mk.tmp
