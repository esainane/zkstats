var dv = (function(dv) {
  ...
  dv.init = function(configloc) {
    d3.json(configloc, function(err, globalConfig) {
      if (err) {
        console.error('Could not fetch config', configloc, ':', err);
        return;
      }
      d3.json(globalConfig.src, function(err, data) {
        if (err) {
          console.error('Could not fetch data', globalConfig.src, ':', err);
          return;
        }
        function dimId(conf) {
